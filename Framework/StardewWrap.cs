...
namespace MineAssist.Framework {
    ...
    class StardewWrap {
        ...
        public static void swapCurrentItemOffset(int offset) {
            swapCurrentItem(Game1.player.CurrentToolIndex + offset, offset >= 0);
        }
        public static void swapCurrentItem(int index, bool rightDirection = true) {
        ...
        public static GamePadState GetRealGamePadState() {
            //return (Game1.input as InputState).GetGamePadState();
            return GamePad.GetState(Game1.playerOneIndex);
        }
        ...
        /// <param name="specialAction">MeleeWeapon has special move action.</param>
        public static void fastUse(int itemIndex, bool specialAction = false) {
            ...
            if (Game1.player.CurrentToolIndex != itemIndex) {
                //swapCurrentItem(itemIndex);
            ...
            if (t is Tool) {
                ...
                if(so.Edibility > 0) {
                    ...
                    Vector2 placementPosition = Utility.GetNearbyValidPlacementPosition(Game1.player, Game1.currentLocation, (Item)Game1.player.ActiveObject, (int)placementGrabTile.X, (int)placementGrabTile.Y);
                    Game1.showGlobalMessage($"POS:{(int)placementPosition.X}, {(int)placementPosition.Y}");
                    //Utility.tryToPlaceItem(Game1.currentLocation, Game1.player.ActiveObject, (int)placementPosition.X, (int)placementPosition.Y);
                    */
                    Game1.pressUseToolButton();
                } else {
                    StardewWrap.inGameMessage($"Fast use of {so.DisplayName} is not supportted");
                }
            } else {
                StardewWrap.inGameMessage($"Fast use of this type is not supportted");
            }
        }
        ...
        public static void endUse(int time, bool specialAction = false) {
            // TODO: weapon use should not be able to interrupted by our fastUse
        ...
        public static void setMove(SDirection m, bool isStart) {
            ...
            if (isStart) {
                switch (m) {
                case SDirection.UP:
                    Game1.upPolling = 1000.0f;
                    break;
                case SDirection.DOWN:
                    Game1.downPolling = 1000.0f;
                    break;
                case SDirection.LEFT:
                    Game1.leftPolling = 1000.0f;
                    break;
                case SDirection.RIGHT:
                    Game1.rightPolling = 1000.0f;
                    break;
                }
            } else {
                switch (m) {
        ...
        public static void updateMove(SDirection m) {
            switch (m) {
            case SDirection.UP:
                Game1.upPolling = 1000.0f;
                break;
        ...
        public static int getCurItemIndex() {
            return Game1.player.CurrentToolIndex;
        }

        public static void setCurItemIndex(int index) {
